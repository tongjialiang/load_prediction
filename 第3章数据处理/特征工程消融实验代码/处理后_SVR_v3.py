...
# 设置参数搜索范围
ajustParameter = "'C':[1e-3,1e-2,1e-1,1,10,100,1000],'gamma':[0.001,0.0001]"
# 设置模型
model = LinearSVR()
# 设置model-name，记录日志用
model_name = 'SVR_v3'
...
class model_SVR_v3:###################################################################################################要修改
    ...
    def do_model(self):
        ...
        if self.way4adjustParameter == "随机搜索":
            print("调参开始")
            grid = RandomizedSearchCV(model, param_dist, cv=None, scoring='neg_mean_squared_error', n_iter=10)
            self.way4cv = 4
