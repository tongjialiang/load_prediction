...
ajustParameter = '''
'criterion' : ['mse'],
'max_depth':range(5, 20) ,  # 10-100 range(5, 50)
'min_samples_split':range(8, 20, 1), 
...
# 设置model-name，记录日志用
model_name = 'RandomForestRegressor_v3'
# 执行小批量数据集 百分比
use_data = False
use_data_ratio = 1
min_use_data = 2000
...
class model_RandomForestRegressor_v3:###################################################################################################要修改
    ...
    def do_model(self):
        ...
        if self.how_long in ["RT","MT","ST"]:
            ...
            if self.how_long in ["RT"]:
                Z_train = data[1][:int(num_data * 0.95), :, [4, 5, 6]]  # 存储除了用电负荷以外的特征
                Z_test = data[1][int(num_data * 0.95):, :, [4, 5, 6]]  # 存储除了用电负荷以外的特征
            elif self.how_long in ["ST"]:  # 0.1905
                Z_train = data[1][:int(num_data * 0.95), :, [5, 9, 11, 12, 13, 14, 15, 16]]  # 存储除了用电负荷以外的特征
                Z_test = data[1][int(num_data * 0.95):, :, [5, 9, 11, 12, 13, 14, 15, 16]]  # 存储除了用电负荷以外的特征
            else:
        ...
        param_dist = {
            ...
            'max_depth': range(5, 20),  # 10-100 range(5, 50)
            'min_samples_split': range(8, 20, 1),
            # 如果某节点的样本数少于min_samples_split，则不会继续再尝试选择最优特征来进行划分。 10  range(8, 20, 1)
            'min_samples_leaf': range(5, 20, 1),  # 如果某叶子节点必须具有的最小样本数。 5 range(5, 20, 1)
            'n_estimators': [10]
        }#c'n_jobs':[1]
        ...
        if self.way4adjustParameter == "随机搜索":
            ...
            self.way4cv = 4
            X_train_t = np.concatenate((X_train, Z_train[:, 0, :]), axis=1)
            small_batch_adjust_parameter(X_train_t, Y_train, use_adjust_parameter, min_use_adjust_parameter,
                                         use_adjust_parameter_ratio)
            print("调参完成")
            self.way4adjustParameter_score = 'MSE'
            self.best_estimator = grid.best_estimator_
        ...
        # with open('d:\\实验记录\\pk\\新息纳入消融-真实值.pk', 'wb+') as f:
        #     pickle.dump(Y_test[:,0], f)
        # f.close()
        # with open('d:\\实验记录\\pk\\新息纳入消融-新息纳入循环预测.pk', 'wb+') as f:
