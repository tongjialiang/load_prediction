...
'''
#设置数据可视化日志存储目录
view_dir="D:\\实验记录\\实验结果\\Transformer_v7\\log\\"

# 设置模型
# model = KNeighborsRegressor()

# 设置model-name，记录日志用
model_name = 'Transformer_v7'
...
class use_Transformer_v7():###################################################################################################要修改
    ...
    def do_model(self):
        ...
        # 随机打散
        print(data[0].shape)  # (5850, 96, 11)
        print(data[1].shape)  # (5850, 20, 11)
        if rsample:
        ...
        if self.how_long in ["RT","MT","ST"]:
            ...
            print(X_train.shape)#(5557, 96)
            print(Y_train.shape)#(5557, 20)
            print(X_test.shape)#(293, 96)
            print(Y_test.shape)#(293, 20)
            print(Y_train[0])
        ...
        class Transformer_Time_series(nn.Module):
            def __init__(self, feature_size=feature_size, num_layers=num_layers, dropout=0.05):
                ...
                self.decoder2 = nn.Linear(lstm_neure, 1)
                self.init_weights()
                self.lstm = nn.LSTM(feature_size, lstm_neure, lstm_layer,
                                    bidirectional=False, batch_first=False, dropout=0.001)
            ...
            def forward(self, src):  # src-->(9,32,1)   (input_window,batchsize,特征数)
                ...
                for i in range(int(y_dim)):
                    ...
                    #     with torch.set_grad_enabled(False):
                    output_temp, _ = self.lstm(output)
                    #print(output_temp.size())#[9, 32, 64]
                    output_temp=output_temp[-1,:,:]#取最后一个数据
                    #print(output_temp.size())# 最后一维是结果 [32, 64]
                    output_temp=output_temp.unsqueeze(0)
                    #print(output_temp.size())#[1, 32, 64]
                    output = torch.cat((output, output_temp), 0)[1:,:,:]
                #print(output.size())#[9, 32, 64]
                    # print(output[-1])
                    # print(output_temp[0])
                output=self.decoder2(output)
                #print(output.size())  #[32, 9]
                return output
        ...
        class PositionalEncoding(nn.Module):
            ...
            def __init__(self, d_model, max_len=5000):
                ...
                self.cnn_layer=nn.Conv1d(  # batchsize*1*30
                    ...
                    kernel_size=3,  # 卷积核大小 in_channels*5*5=1*5*5  3
                    stride=1,  # 步长  1
            ...
            for epoch in range(num_epochs):
                ...
                for phase in ['train', 'valid']:
                    ...
                    for inputs, labels in dataloaders[phase]:
                        ...
                        # 计算损失
                        running_loss += loss.item() * inputs.size(1)#[9, 32, 1]
...
#############################循环预测###############################################
            ...
            Y_predict_res=Y_predict_res.permute(1, 0)
            Y_test_real=Y_test_real.permute(1, 0)
#############################循环预测###############################################
            ...
            # print("MSE")
            # self.MSE_score = mean_squared_error(Y_test_seq.squeeze().cpu().numpy(), Y.cpu().numpy())
            # print(self.MSE_score)
