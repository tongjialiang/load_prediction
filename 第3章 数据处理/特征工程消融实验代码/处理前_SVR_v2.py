...
class model_SVR_v2:###################################################################################################要修改
    ...
    def do_model(self):
        ...
        if self.way4adjustParameter == "随机搜索":
            print("调参开始")
            grid = RandomizedSearchCV(model, param_dist, cv=None, scoring='neg_mean_squared_error', n_iter=20)
            self.way4cv = 4
            small_batch_adjust_parameter(X_train, Y_train, use_adjust_parameter, min_use_adjust_parameter,
                                         use_adjust_parameter_ratio)
            print("调参完成")
        ...
        if self.way4adjustParameter == "贝叶斯搜索":
            grid = BayesSearchCV(model, [param_dist], cv=None, scoring='neg_mean_squared_error', n_iter=10)
            self.way4cv = 4
            small_batch_adjust_parameter(X_train, Y_train, use_adjust_parameter, min_use_adjust_parameter,
                                         use_adjust_parameter_ratio)
