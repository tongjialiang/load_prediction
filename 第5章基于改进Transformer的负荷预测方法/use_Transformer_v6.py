...
'''
#设置数据可视化日志存储目录
view_dir="D:\\实验记录\\实验结果\\Transformer_v6\\log\\"

# 设置模型
# model = KNeighborsRegressor()

# 设置model-name，记录日志用
model_name = 'Transformer_v6'
    ...
    def do_model(self):
        ...
        class Transformer_Time_series(nn.Module):
            def __init__(self, feature_size=feature_size, num_layers=num_layers, dropout=0.1):
                ...
                self.decoder2 = nn.Linear(lstm_neure*2, x_dim)
                self.init_weights()
            ...
            def forward(self, src):  # src-->(9,32,1)   (input_window,batchsize,特征数)
                ...
                output=output[-1,:,:]#取最后一个数据
                #print(output.size())# 最后一维是结果 [32, 20]
                output=self.decoder2(output)
                #print(output.size())  #[32, 9]
                output=output.unsqueeze(-1).permute(1,0,2)
                #print(output.size())
                return output
