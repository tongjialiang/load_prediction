...
ajustParameter ='略'
#设置数据可视化日志存储目录
view_dir="D:\\实验记录\\实验结果\\Transformer_DA\\log\\"
...
#print(random.choice(np.linspace(0,1,20)))

#np.random.seed()
w_choice=random.choice([[.53,.47,0],[.71,.17,.12],[.79,.06,.15],[.55,.24,.21],[.84,.01,.15],[.82,.11,.07],[.87,.08,.05],[.63,.31,.06],[.55,.33,.12],[1,0,0]])
w1,w2,w3=w_choice
# w1=random.choice(np.linspace(0.5,0.9,20))
# w1=round(w1, 2)
# w2=random.choice(np.linspace(0,1-w1,20))
# w2=round(w2, 2)
# w3=1-w1-w2
# w3=round(w3, 2)
# print(w1,w2,w3)
...
myopt="AdamW"
#学习率
#lr=random.choice([0.005,0.008,0.01,0.001])#lr=0.005
lr=random.choice([0.005,0.008,0.01,0.001])
# print(lr)
# 1/0
#Transfomer，Embedding的维数 64 128 256 32
feature_size=random.choice([64,32,128,100,150]) #64 best
#feature_size=random.choice([256,300,400,512])
#lstm的神经元
lstm_neure = feature_size
lstm_layer =random.choice([1,1,1,2])# 1
#lstm_layer =random.choice([2,2,3,3])
#TransformerEncoder的层数 234
num_layers=random.choice([1,1,1,2])#1
#num_layers=random.choice([2,2,3,3])
#使用多少头的Multi-Head Attention 4 8  10 16
nhead=random.choice([1,1,2,2])#1
#nhead=random.choice([2,2,3,4])
#解码器的FC维度
fc_decoder_l1=random.choice([10,16,32,22])#16
#fc_decoder_l1=random.choice([32,16,64,64])
fc_decoder_l2=random.choice([10,16,32,22])#16
#fc_decoder_l2=random.choice([32,16,64,64])
isBatchNorm1d=True #解码器的FC层之后是否使用BatchNorm1d
FC_D_l1=random.choice([10,16,32,22]) #鉴别器FC层的第1层维度16
#FC_D_l1=random.choice([32,16,64,64])
FC_D_l2=random.choice([10,16,32,22]) #鉴别器FC层的第2层维度16
#领域 聚类-3 地区-2 行业-1
#领域 聚类-4 地区-3 行业-2 负荷特性聚类-1
domain=random.choice([-2])
#跑多少个epochs
num_epochs=3000#100
    ...
    def do_model(self):
        ...
        class Transformer_Time_series(nn.Module):
            ...
            def forward(self, src,other_features):  # src-->[70, 512, 1]   (input_window,batchsize,特征数)
                ...
                # 1/0
                label_forecast = self.Discriminator(src)
                ...
                # 1/0
                return output_y,label_forecast,src #预测值，领域标签的预测值，encoder的输出
