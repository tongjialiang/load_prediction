...
###########################################################
frompath="C:\\myfiles\\毕业论文\\论文撰写\\实验结果与配图\\第6章电力数据质量分析与异常监控系统\\聚类DBscan调参结果_小颗粒度_实时预警.xls"
topath="C:\\myfiles\\毕业论文\\论文撰写\\实验结果与配图\\第6章电力数据质量分析与异常监控系统\\半径与离群点个数的关系曲线.jpg"
data=read_excel(frompath)
print("打印有多少列")
print(data["聚类DBscan调参结果_小颗粒度_实时预警"].columns)########
sns.set_style('darkgrid',{'font.sans-serif':['SimHei','Arial']})
# plt.rcParams['figure.dpi'] = 1000
sns.set_palette(palette="dark")
x=data["聚类DBscan调参结果_小颗粒度_实时预警"]["半径"]########
y=data["聚类DBscan调参结果_小颗粒度_实时预警"]["离群点个数"]########
#plt.xticks(rotation=20)#######倾斜角度
plt.xlabel("半径")
plt.ylabel("离群点个数")
# print(y.sort_values().index.value)
#sns.barplot(x,y,order=x[y.sort_values(ascending=False).index])#降序
sns.lineplot(x,y,label="离群点个数")
plt.legend()
