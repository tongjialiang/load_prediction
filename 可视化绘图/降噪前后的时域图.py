...
for index,i in enumerate(data[0]):
    ...
    if index==5:
        sns.lineplot(data=ts_ifft[:5000], label='降噪后的时域图')
        #plt.plot(ts_ifft[:5000], label='降噪后')
        plt.rcParams['font.sans-serif'] = ['SimHei']
        #plt.title("某用电公司降噪后的时域图")
        plt.xlabel("时间")
        plt.ylabel("电力负荷(归一化)")
        plt.legend()
        plt.savefig("C:\\myfiles\\毕业论文\\论文撰写\\实验结果与配图\\第三章数据集划分方式的研究\\某用电公司降噪后的时域图", dpi=1000)
        plt.show()
    if index == 5:
        sns.lineplot(data=ts[:5000], label='降噪前的时域图')
        plt.rcParams['font.sans-serif'] = ['SimHei']
        #plt.title("某用电公司降噪前的时域图")
        plt.xlabel("时间")
        plt.ylabel("电力负荷(归一化)")
        plt.legend()
        plt.savefig("C:\\myfiles\\毕业论文\\论文撰写\\实验结果与配图\\第三章数据集划分方式的研究\\某用电公司降噪前的时域图", dpi=1000)
        plt.show()
