...
'''
#设置数据可视化日志存储目录
view_dir="C:\\实验记录\\实验结果\\Transformer\\log\\"
...
class use_Transformer_v1():###################################################################################################要修改
    ...
    def do_model(self):
        ...
        class Transformer_Time_series(nn.Module):
            ...
            def forward(self, src):  # src-->(100,10,1)   (input_window,batchsize,特征数)
                if self.src_mask is None or self.src_mask.size(0) != len(src):
                    device = src.device
                    mask = self._generate_square_subsequent_mask(len(src)).to(device)
                    self.src_mask = mask
                # print(src.shape)  #torch.Size([100, 10, 1])
                src = self.pos_encoder(src)  # 100,10,1-->100,10,250
                output = self.transformer_encoder(src, self.src_mask)  # , self.src_mask)
                # print(output.size())#[100, 10, 250]
                # 1/0
                output = self.decoder(output)  # [100, 10, 1]
                # print(output.size())
                # 1/0
                return output
        ...
        class PositionalEncoding(nn.Module):
            ...
            def forward(self, x):  # x-->(100,10,1)   (input_window,batchsize,特征数)
                res = x + self.pe[:x.size(0), :]
                # print(res.shape)#100,10,250
                return res  # (100,10,1)+(100,1,250)
        #模型初始化
        def model_init():
        ...
        with torch.set_grad_enabled(False):
            ...
            model_best.cpu()
            model_best.eval()
            # print(X_test.size())#[12, 30, 1]
            # print(X_test.permute(1,0,2).size())#[30, 12, 1]
            Y_predict=model_best(X_test.permute(1,0,2))

            #print(Y_predict.size())#[30, 12, 1]
            Y_predict=torch.squeeze(Y_predict)

            #print(Y_predict.size())#[30, 12]
            Y_predict=Y_predict[-1,:]
            #print(Y_predict.size())
            print(Y_predict.size())#11
            print(Y_test.size())
            print(Y_predict)  # 11
            print('-'*30)
            print(Y_test)
