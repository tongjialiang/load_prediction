...
class model_Ridge_v3:###################################################################################################要修改
    ...
    def do_model(self):
        ...
        if self.way4adjustParameter == "随机搜索":
            print("调参开始")
            grid = RandomizedSearchCV(model, param_dist, cv=None, scoring='neg_mean_squared_error', n_iter=12)
            self.way4cv = 4
            small_batch_adjust_parameter(X_train, Y_train, use_adjust_parameter, min_use_adjust_parameter,
                                         use_adjust_parameter_ratio)
            print("调参完成")
        ...
        # f.close()
        with open('D:\\myfiles\\毕业论文\\论文撰写\\实验结果与配图\\第8章基于改进Transformer模型的负荷预测方法\\绘制预测曲线与损失曲线\\'+'_'+self.how_long+'_Ridge预测值.pk', 'wb+') as f:
