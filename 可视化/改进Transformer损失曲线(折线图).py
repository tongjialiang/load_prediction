...
topath="d:\\myfiles\\毕业论文\\论文撰写\\实验结果与配图\\第8章基于改进transformer模型的负荷预测方法\\"+"改进Transformer模型的损失曲线.jpg"

sns.set_style('darkgrid',{'font.sans-serif':['SimHei','Arial']})

resource="D:\\myfiles\\毕业论文\\论文撰写\\实验结果与配图\\第8章基于改进transformer模型的负荷预测方法\\绘制预测曲线与损失曲线\\"

with open(resource+'_MTtrain_losses.pk', 'rb') as f:
  y1 = pickle.load(f)
f.close()
print(y1)
with open(resource+'_MT_valid_losses.pk', 'rb') as f:
  y2 = pickle.load(f)
f.close()
...
sns.lineplot(x,y1,ci='red',label="训练集误差")
sns.lineplot(x,y2,ci='black',label="测试集误差")
