...
import pprint
import calendar
def do_GetClusteringXandBusname_long():
    clustering_samples={} #存放聚类样本X的字典，键：公司名称 值：X=[均值、标准差]
    business_name=[]
    with open('c:\\实验记录\\pk\\dict_BusTypeOrArea_MeanAndStd_method2(normal).pk', 'rb') as f:
        data = pickle.load(f)
    #print(len(data))#9661
    #print(data)
    x=[] #聚类样本
...
# do_GetClusteringXandBusname_long()
