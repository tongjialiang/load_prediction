...
'''
#设置数据可视化日志存储目录
view_dir="D:\\实验记录\\实验结果\\Transformer_v8\\log\\"

# 设置模型
# model = KNeighborsRegressor()

# 设置model-name，记录日志用
model_name = 'Transformer_v8'
            ...
            def forward(self, src,other_features):  # src-->(9,32,1)   (input_window,batchsize,特征数)
                for i in range(int(y_dim)):
                    ...
                    if self.src_mask is None or self.src_mask.size(0) != len(src2):
                        ...
                        self.src_mask = mask
                        # print(src2.shape)  #[11, 32, 1]
                        # print(mask.shape)#[11, 11]

                    src2 = self.pos_encoder(src2)  #  # (9,32,1)+([9, 32, 64])-->[9, 32, 64]
                    #print(src2.shape)#[11, 32, 64]
                    output = self.transformer_encoder(src2, self.src_mask)  # , self.src_mask)
                    #print(output.size())#[11, 32, 64]
                    ...
                    #     with torch.set_grad_enabled(False):
                    output_temp, _ = self.lstm(output)
                    #print(output_temp.size())#[11, 32, 64]
                    ...
                    output_temp = self.decoder2(output_temp)
                    #print(output_temp.size())#[32, 1]  得到一天的预测值
                    output_temp=output_temp.squeeze().unsqueeze(0)
                    #print(output_temp.size())#[1,32]
                    #print(output_temp)
                    src=src.squeeze()#[6, 32, 1]-->[6, 32]
                    src = torch.cat((src, output_temp), 0)[1:,:]
                    src=src.unsqueeze(-1)
                    #print(src.size())#[6, 32, 1]
                return src
